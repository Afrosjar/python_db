...
class Database:
    """ Предназначен для управления создаваемой базой данных. Для начала работы необходимо ввести название базы данных,
     имя пользователя и пароль Postgre"""
    ...
    def get_connection(self):
        """Функция проверяет есть ли связь с базой данных, открывает ее и отдает информацию об SQL версии,
         после использования функции не забудьте выполнить команду connection.close() """
        try:
    ...
    def update_table(self, new, old):
        ...
        ''', (71111111, 79251332217), где 71111111 - номер телефона на который будет изменен 79251332217"""
        connection = self.get_connection()
        cursor = connection.cursor()
        try:
...
if __name__ == "__main__":
    ...
    db_user.delete_info("""DELETE FROM clients
                            ...
                            WHERE c.email = 'afros@mail.ru'
                            LIMIT 1);
                            """)  # Выполнит каскадное удаление клиента из всех связанных отношений по email.
    #
    print(db_user.get_id_clients('prot@ya.ru'))  # Проверка функции поиска ID по любым данным клиента
